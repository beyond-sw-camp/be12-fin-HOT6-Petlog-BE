        ...
        for (Long roomId : roomIds) {
            ...
            for (int i = 0; i < count; i++) {
                scheduleBatch.add(new Object[]{
                        roomId,
                        "일정 " + UUID.randomUUID(),                     // s_title
                        "메모 내용 " + UUID.randomUUID(),                 // s_memo
                        false,                                           // recurring
                        null,                                            // repeat_cycle
                        "place_" + rand.nextInt(1000),                   // place_id
                        false,                                           // is_deleted
                        0,                                               // repeat_count
                        null,                                            // repeat_end_at
    ...
    private void insertHashtags(List<Long> roomIds, int countPerRoom) {
        ...
        for (Long roomId : roomIds) {
            for (int i = 0; i < countPerRoom; i++) {
                batch.add(new Object[]{roomId, "#태그" + rand.nextInt(1000)});
            }
        }
